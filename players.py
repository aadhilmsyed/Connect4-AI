...
class alphaBetaAI(connect4Player):
    ...
    def maxValue(self, env, depth, alpha, beta):
        ...
        # Terminal State Test
        if depth == 0 or len(indices) == 0: return self.eval_board(env), None
        
        # Successor Function (Priority Columns)
#        indices = self.heuristic(indices)
...
#        indices = self.heuristic(indices)
    ...
    def heuristic(self, indices):
        priority = [3, 0, 6, 1, 2, 4, 5]
        result = []
        for col in priority:
            if col in indices: result.append(col)
        return result
